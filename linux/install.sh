...
if [ $(id -u) != "0" ];
then
	echo -e "|   Error: You need to be root to install the monitor-agent\n|"
...
if [ -f /etc/monitor-agent/agent.sh ]
then
	# Remove agent dir
	rm -Rf /etc/monitor-agent

	# Remove cron entry and user
	if id -u monitor-agent >/dev/null 2>&1
	then
		(crontab -u monitor-agent -l | grep -v "/etc/monitor-agent/agent.sh") | crontab -u monitor-agent - && userdel monitor-agent
	else
		(crontab -u root -l | grep -v "/etc/monitor-agent/agent.sh") | crontab -u root -
	fi
fi
...
# Download agent
echo -e "|   Downloading agent.sh to /etc/monitor-agent\n|\n|   + $(wget -nv -o /dev/stdout -O /etc/monitor-agent/agent.sh --no-check-certificate https://raw.github.com/LIznzn/monitor-agent/master/linux/agent.sh)"

if [ -f /etc/monitor-agent/agent.sh ]
then
	# Create auth file
	echo "$1" > /etc/monitor-agent/auth.conf
	
	# Create user
	useradd monitor-agent -r -d /etc/monitor-agent -s /bin/false
	
	# Modify user permissions
	chown -R monitor-agent:monitor-agent /etc/monitor-agent && chmod -R 700 /etc/monitor-agent

	# Configure cron
	crontab -u monitor-agent -l 2>/dev/null | { cat; echo "*/3 * * * * bash /etc/monitor-agent/agent.sh > /etc/monitor-agent/cron.log 2>&1"; } | crontab -u monitor-agent -
	
	# Show success
	echo -e "|\n|   Success: The monitor-agent agent has been installed\n|"
	...
	# Show error
	echo -e "|\n|   Error: The monitor-agent agent could not be installed\n|"
fi
