...
if [ -f /etc/monitor-agent/auth.conf ]
then
	auth=($(cat /etc/monitor-agent/auth.conf))
else
	echo "Error: Authentication conf is missing."
	exit 1
fi
...
host_name=$(prep "$(cat /proc/sys/kernel/hostname)")
...
if [ -e /etc/monitor-agent/data.log ]
then
	data=($(cat /etc/monitor-agent/data.log))
...
# System load cache
echo "$time $cpu $io $idle $rx $tx" > /etc/monitor-agent/data.log
